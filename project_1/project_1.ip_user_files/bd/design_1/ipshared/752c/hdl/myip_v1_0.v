	...
	// Add user logic here
       ...
       (* keep = "true" *) wire st_out;
        ...
        Fourbyfour my_fourbyfour( 
            ...
            .dout(kernel_out),
            .st_out(st_out), .done(core_done)
        );
	

        reg [5:0] out_cnt;
		reg [31:0] repeat_copy = 0;
        always @(posedge s00_axi_aclk) begin
            if (rst || core_rst)                    out_cnt <= 0;
		...
		always @(posedge s00_axi_aclk) begin
			...
			else if (opst_delay)begin
				if(core_done && (repeat_copy < repeat_o) )	begin 
					core_rst <= 1'b1;
					core_op_st <= 1'b0;
					repeat_copy <= repeat_copy + 1;
					
				end
				else if(core_done && repeat_copy == repeat_o) begin
					core_op_st <= 1'b0;
					done <= 1'b1;
					
				end
				else if(!core_done && core_rst) begin
					core_rst <= 1'b0;
					core_op_st <= 1'b1;
				end
			end
        ...
        assign bram_out_addr = st_out ? out_cnt : core_done ?  addra0 >> 3 : 0 ;
